...
## 怪癖 6：颜色只认 6 位 hex，带 alpha 的会让整份导入失败

**症状**：拖入 SVG 时弹红框 `Invalid color: #RRGGBBAA`（附 "Please email support@axure.com and attach the SVG or RP file"），**整份 SVG 一个形状都进不来**——Axure 遇到第一个非法颜色就整体拒绝。

**原因**：Axure for Mac 的颜色解析器不接受**带透明度的 hex**（8 位 `#RRGGBBAA`、4 位 `#RGBA`）。

**实测结论（2026-06-03 探针验证）**：

| 写法 | 结果 |
|---|---|
| `#RRGGBB`（6 位）| ✅ 标准，永远 OK |
| `#RGB`（3 位，如 `#000`）| ✅ **实测可用**，Axure 接受，无需补全 |
| `#RRGGBBAA`（8 位带 alpha）| ❌ 报 Invalid color，整份失败 |
| `#RGBA`（4 位带 alpha）| ❌ 同上 |

**修复**：透明度用独立属性表达，别塞进 hex：

```xml
<!-- ❌ Axure 拒绝 -->  <rect fill="#10B98122"/>
<!-- ✅ 等价且合法 -->  <rect fill="#10B981" fill-opacity="0.133"/>   <!-- 0x22/0xFF≈0.133 -->
```

`stroke` 同理用 `stroke-opacity`。

**已自动化**：`lib/primitives.mjs` 的 `colorAttr()`（被 `rect()` / `txt()` 调用）会把任何带 alpha 的 hex 自动拆成 `颜色 + *-opacity`，3 位 / 6 位原样放行。所以只要颜色经过 `rect()` / `txt()`，就不会再踩这个坑——别绕过它们直接手写 `fill="#RRGGBBAA"`。
